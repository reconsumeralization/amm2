...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!session?.user) {
      return createErrorResponse('Unauthorized', 'AUTHENTICATION_ERROR', 401)
    }

    // Check if user has content access
    const allowedRoles = ['admin', 'manager', 'barber', 'stylist']
    if (!allowedRoles.includes(session.user.role || '')) {
      return createErrorResponse('Insufficient permissions', 'AUTHORIZATION_ERROR', 403)
    }
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Create content with enhanced data
...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (!canEdit) {
      return createErrorResponse('Insufficient permissions to edit this content', 'AUTHORIZATION_ERROR', 403)
    }
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    if (!canDelete) {
      return createErrorResponse('Insufficient permissions to delete this content', 'AUTHORIZATION_ERROR', 403)
    }
