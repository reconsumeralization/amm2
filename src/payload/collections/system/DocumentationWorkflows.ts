...
export const DocumentationWorkflows: CollectionConfig = {
  ...
  timestamps: true,
  access: {
    read: ({ req: { user } }: { req: { user: any } }) => ['system_admin', 'BarberShop_owner', 'developer'].includes(user?.role || ''),
    create: ({ req: { user } }: { req: { user: any } }) => ['system_admin', 'BarberShop_owner'].includes(user?.role || ''),
    update: ({ req: { user } }: { req: { user: any } }) => ['system_admin', 'BarberShop_owner'].includes(user?.role || ''),
    delete: ({ req: { user } }: { req: { user: any } }) => ['system_admin'].includes(user?.role || ''),
  },
  hooks: {
    beforeChange: [
