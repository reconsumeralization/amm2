  ...
  hooks: {
    beforeChange: [
      async ({ data, req, operation }) => {
        ...
        if (data.pricing?.costPrice && data.pricing?.basePrice) {
          const costPrice = data.pricing.costPrice;
          const salePrice = data.pricing.basePrice;
          const profit = salePrice - costPrice;
          const margin = costPrice > 0 ? (profit / costPrice) * 100 : 0;
          const markup = costPrice > 0 ? (profit / costPrice) * 100 : 0;

          data.profitMargin = Math.round(margin * 100) / 100; // Round to 2 decimal places
          data.markupPercentage = Math.round(markup * 100) / 100;
        }

        // Auto-set low stock threshold if not provided
        if (data.stock !== undefined && data.lowStockThreshold === undefined) {
          data.lowStockThreshold = Math.max(5, Math.floor(data.stock * 0.1)); // 10% of stock or minimum 5
        }

        // Validate pricing consistency
        if (data.pricing?.salePrice && data.pricing?.basePrice) {
          if (data.pricing.salePrice > data.pricing.basePrice) {
            throw new Error('Sale price cannot be higher than base price');
          }
        }

        // Auto-categorize product based on keywords
