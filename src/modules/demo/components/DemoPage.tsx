...
export default function DemoPage() {
  const [ThemeShowcaseWidget, setThemeShowcaseWidget] = useState<any>(null)
  const [themeModuleLoaded, setThemeModuleLoaded] = useState(false)

  useEffect(() => {
    const loadThemeWidget = async () => {
      try {
        const widget = await ModuleRegistry.loadWidget('theme', 'ThemeShowcaseWidget')
        if (widget) {
          setThemeShowcaseWidget(() => widget)
          setThemeModuleLoaded(true)
        }
      } catch (error) {
        console.warn('Theme module not available:', error)
        setThemeModuleLoaded(false)
      }
    }
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="text-center mb-12">
          ...
          <p className="text-xl text-gray-600 max-w-3xl mx-auto">
            Use these demo accounts to test different user roles and permissions in the Modern Men barber shop application.
          </p>
        </div>
        ...
        {themeModuleLoaded && ThemeShowcaseWidget && (
          <div className="mt-16">
            <Card className="p-6">
              <CardHeader>
                <CardTitle className="text-2xl">Theme Showcase</CardTitle>
                <CardDescription>
                  Explore interactive demos and examples from the theme module
                </CardDescription>
              </CardHeader>
              <CardContent>
                <ThemeShowcaseWidget showHeader={false} defaultTab="overview" />
              </CardContent>
            </Card>
          </div>
        )}
            ...
            <Card className="p-6 border-dashed">
              <CardContent className="text-center py-8">
                ...
                <p className="text-gray-600 mb-4">
                  The theme showcase is currently unavailable. Interactive demos and examples
                  will appear here when the theme module is loaded.
                </p>
